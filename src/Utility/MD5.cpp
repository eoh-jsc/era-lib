...
char* MD5::makeDigest(const unsigned char* digest, int len, char* result) {
    char* md5str = result;
    if (md5str == NULL) {
        md5str = (char*)malloc(sizeof(char) * ((len * 2) + 1));
    }
...
# define SET(n) \
    (ctx->block[(n)] = \
    (MD5_u32plus_t)ptr[(n) * 4] | \
    ((MD5_u32plus_t)ptr[(n) * 4 + 1] << 8) | \
    ((MD5_u32plus_t)ptr[(n) * 4 + 2] << 16) | \
    ((MD5_u32plus_t)ptr[(n) * 4 + 3] << 24))
# define GET(n) \
    (ctx->block[(n)])
#endif
...
const void* MD5::body(void* ctxBuf, const void* data, size_t size) {
    MD5_CTX_t* ctx = (MD5_CTX_t*)ctxBuf;
    const unsigned char *ptr;
    MD5_u32plus_t a, b, c, d;
    MD5_u32plus_t saved_a, saved_b, saved_c, saved_d;
...
void MD5::MD5Update(void* ctxBuf, const void* data, size_t size) {
    MD5_CTX_t* ctx = (MD5_CTX_t*)ctxBuf;
    MD5_u32plus_t saved_lo;
    MD5_u32plus_t used, free;
    ...
    if (used) {
        ...
        size -= free;
        MD5::body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = MD5::body(ctx, data, size & ~(size_t)0x3f);
        size &= 0x3f;
    }
...
unsigned char* MD5::makeHash(char* arg) {
    MD5_CTX_t context;
    unsigned char* hash = (unsigned char*)malloc(16);
    MD5::MD5Init(&context);
    MD5::MD5Update(&context, arg, strlen(arg));
    MD5::MD5Final(hash, &context);
    return hash;
}

unsigned char* MD5::makeHash(char* arg, size_t size) {
    MD5_CTX_t context;
    unsigned char* hash = (unsigned char*)malloc(16);
    MD5::MD5Init(&context);
    MD5::MD5Update(&context, arg, size);
    MD5::MD5Final(hash, &context);
    return hash;
}
