#ifndef INC_MVP_FLASH_STM32_HPP_
#define INC_MVP_FLASH_STM32_HPP_
...
char* MVPFlash::readFlash(const char* filename) {
    ...
    while (this->readFlashUntil('/')) {
        char* buf = (char*)MVP_MALLOC(this->getSizeFromPosition() + 1);
		if (buf == nullptr) {
			return nullptr;
		}
        ...
        free(buf);
        buf = (char*)MVP_MALLOC(this->getSizeFromPosition() + 1);
		if (buf == nullptr) {
			return nullptr;
		}
        this->readFlashUntil('\0', buf);
...
template <typename T>
size_t MVPFlash::readFlash(const char* key, T& buffer) {
...
void MVPFlash::writeFlash(const char *filename, const char* buffer) {
    if (buffer == nullptr) {
        return;
    }
...
template <typename T>
size_t MVPFlash::writeFlash(const char* key, const T& value) {
    ...
    bool MVPFlash::readFlashUntil(const char c, char* buffer) {
        size_t length {0};
        for (size_t i = this->position; i < EEPROM.length(); ++i) {
            if (buffer != nullptr) {
    ...
    bool MVPFlash::clearFlashUntil(const char c) {
        for (size_t i = this->position; i < EEPROM.length(); ++i) {
            if (eeprom_buffered_read_byte(i) == c) {
                this->position = i;
                return true;
    ...
    void MVPFlash::flashMove(size_t dst, size_t src) {
    ...
    size_t MVPFlash::getSizeFromPosition() {
        char c {0};
        size_t size {0};
        do {
...
size_t MVPFlash::getSize(const char* filename) {
    char c {0};
    size_t size {0};
    uint16_t address = atof(filename);
    do {
...
#endif /* INC_MVP_FLASH_STM32_HPP_ */